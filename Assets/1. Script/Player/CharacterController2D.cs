...
public class CharacterController2D : MonoBehaviour
{
	...
	public void Move(float move, bool jump, bool doubleJumpSkill)
	{
		//canDash = false;
		isDashing = false;
		isWallSliding = false;
		...
		if (canMove) {
			...
			*/
			...
			else if (!m_Grounded && jump && canDoubleJump && !isWallSliding && doubleJumpSkill)
			{
				//Debug.Log("canDoubleJump = false");
				canDoubleJump = false;
	...
	public void ApplyDamage(float damage, Vector3 position) 
	{
		if (!invincible)
		{
			...
			m_Rigidbody2D.AddForce(damageDir * 3);
			if (life <= 0)
			{
